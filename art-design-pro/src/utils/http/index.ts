...
axiosInstance.interceptors.response.use(
  (response: AxiosResponse<Http.BaseResponse>) => {
    ...
    // 兼容后端返回 message 或 msg
    const errorMsg = msg || message

    // 兼容多种成功状态码：0, 200, undefined（直接返回数据的情况）
    if (code === 0 || code === ApiStatus.success || code === 200 || code === undefined)
      return response
